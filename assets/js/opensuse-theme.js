...
function headerChanges() {
  $('header').css({
    // background: moreInfoOpened ? '#6da741' : 'transparent',
    top: '0px',
    height: '45px',
    padding: '8px',
    'z-index': '1000'
...
function osMoreInformation(os) {
  ...
  if(osSelectedTitle == "Genomics") {
    osSelectedText = "Genomics is a new and very active application area of computer science. " +
                     "The past ten years there has been an explosion of genomics data -- the entire " +
                     "DNA sequences of several organisms, including human, are now available. " +
                     "These are long strings of base pairs (A,C,G,T) containing all the information necessary for an organism's development and life. " +
    ...
    osSelectedText = "Neuroscience is the study of the nervous system, which includes the brain and the spinal cord. " +
    "It's a broad term encompassing numerous, more specific disciplines, including cognitive, clinical, and developmental neuroscience.";
  }
  ...
  function createMoreInformationArea () {
    information = '<div class="text-center" id="more-information-os">'
                  ...
                  + osSelectedTitle
                  + '</h1>'
                  + '<div class="wow fadeInUp" style="color: white">'
                  + osSelectedText
                  + '</div>'
                  + '<br/>'
...
function backToMainPageOs (os) {
  ...
  $('#opensuse-os').removeClass('animated bounceOut');
  $('#opensuse-os').css({
    height: containerHeight + 'px'
  });
  $('#opensuse-os').css("background-color", "transparent");
